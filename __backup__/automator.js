...
function automator(options) {
    ...
    function getNextActionTime(start, repeat) {
        ...
        //this is for the DST calculation at the end of the function
        var initialOffset = dateStart.getTimezoneOffset();
        ...
        // Check if nextTime is before start due to DST shift
        var newOffset = nextTime.getTimezoneOffset();
        if (newOffset !== initialOffset) {
            nextTime.setHours(nextTime.getHours() + (initialOffset - newOffset) / 60);
        }
